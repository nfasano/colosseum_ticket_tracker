...
Figure 3 below shows the full experience ticket availability for the same two weeks as the figures above. The first plot shows the availability of tickets when the website was queried on the release date (30 days in advance). The left plot shows all types of full experience tickets while the right plot only shows availability for full experience tickets that include underground access. The empty times lost for the underground access are expected because of the time gaps in available ticket entry times as we saw in Figure 1 above. As with the regular experience tickets, a second batch of tickets is released 7 days in advance and the day before your visit (Figure 5). These tickets tend to not sell out as fast as the ones released 30 days before your visit. I have never seen the availability of tickets from 29 days - 8 days or 6-2 days before the ticket date, so I would not even bother checking the official website for these tickets.
...
The code in this repo can be easily adapted to implement this strategy algorithmically. One could even launch several instances simultaneously (in parallel) to maximize the chances of getting the calendar day to turn green. In words the algorithm will be something like this:
1) Launch several instances of the website and repeatedly refresh the webpage until you detect the calendar turn from green to red. Click the calendar day and if there are any available tickets, then try to purchase them, but the goal with this first step is to not necessarily get tickets at the first time slot, but rather just to see the calendar date turn green and hence become clickable for all remaining time slots.
